...
class DataUtil:
    ...
    def __getFileLines(self, filename):
        with open(filename) as f:
            lines = f.readlines()
            lines = [l.replace("\n","").replace("\r","").strip() for l in lines]
    ...
    def __readTaggedData(self,mode):
        if mode==constants.TRAIN:
            ...
            lines = self.__getFileLines(self.dev_file) + self.__getFileLines(self.train_file)
        ...
        for i,line in enumerate(lines):
            ...
            for j,word_tag in enumerate(line):
                ...
                if word not in word_count:
                    ...
                    word_count[word]+=1
            tag_count[constants.BOL_TAG]+=1 #begin of line tag for each line
