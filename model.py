...
class Model:
    def __init__(self, mode=constants.TRAIN, laplaceParam=0.005):
        self.model_file = "hmmmodel.txt"
        self.tags = []
        self.vocab = []
        self.emission_probs = {}
        self.transition_probs = {}
        self.laplace_param = laplaceParam
        if mode == constants.TEST:
            self.__fetchModel()

    def __laplace(self, c1, c2, V):
        return (c1+self.laplace_param)/(c2 + (self.laplace_param*V))
        ...
        for prev_tag in self.tags:
            for tag in self.tags:
                if not self.transition_probs[prev_tag][tag]:
                    self.transition_probs[prev_tag][tag] = self.__laplace(
                        0, tag_count[prev_tag], totalTags)

        # for tag in self.tags:
        #     print(tag,self.emission_probs[tag][constants.UNKNOWN_WORD])
        self.extractOpenClassTags(word_tag_count)
        print("Tags : ", len(self.tags), "OpenClass Tags : ", len(
            self.open_class_tags), "Vocab : ", len(self.vocab))
